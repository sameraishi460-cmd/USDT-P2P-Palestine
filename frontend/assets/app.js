/** USDT P2P Palestine — Frontend Client */
...
function formatNum(n, dec) { dec = dec || 2; return Number(n || 0).toFixed(dec); }
...
function highlightNav(id) { document.querySelectorAll('.bottom-nav a').forEach(function(a) { a.classList.toggle('active', a.id === id); }); }
...
// Page-specific helper: show/hide auth-dependent elements
function updateAuthUI() {
  var user = getUser();
  var authEls = document.querySelectorAll('[data-auth]');
  var noAuthEls = document.querySelectorAll('[data-noauth]');
  authEls.forEach(function(el) { el.style.display = user ? '' : 'none'; });
  noAuthEls.forEach(function(el) { el.style.display = user ? 'none' : ''; });
}
